...
// Definição da interface para evitar erros de tipagem
...
export default function SimuladorPage({ searchParams }: { searchParams: { period?: string } }) {
  const periodData = searchParams?.period || '90d';
  const months = (({ '30d': 1, '60d': 2, '90d': 3, '6m': 6, '1y': 12, '2y': 24 } as Record<string, number>)[periodData]) || 3;
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/api/company-stats').then(r => r.ok ? r.json() : null), // Alterado para null para facilitar checagem
    ]).then(([cData, sData]: [any[], CompanyStats | null]) => {
      ...
      // Checagem segura das propriedades de sData
      if (sData && typeof sData.totalRevenue === 'number') {
  ...
  function getRows(): [string, string, string][] {
    ...
    if (tab === 'pipeline') {
      return [
        ...
        [`${pipelineClients.length} oportunidades`, pipelineClients.filter((c:any)=>c.isRecurring).length + ' rec. / ' + pipelineClients.filter((c:any)=>!c.isRecurring).length + ' pont.', 'text-blue-600'],
  ...
  function getAlert(): { t: 'ok' | 'warn' | 'danger'; msg: string } | null {
    ...
    if (tab === 'pipeline') { return pipelineRev > 0 ? (b + pipelineRev > 0 ? { t: 'ok', msg: `Convertendo todo o pipeline: ${BRL(b + pipelineRev)}/mês.` } : { t: 'warn', msg: `Mesmo com o pipeline (${BRL(pipelineRev)}), déficit persiste.` }) : { t: 'warn', msg: 'Nenhum cliente no pipeline ainda.' }; }
  ...
  return (
    <div className="space-y-5">
        ...
        {TABS.map(([id, lbl, icon]) => (
          <button key={id} onClick={() => switchTab(id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-medium transition-all border ${
              tab === id ? 'bg-[#1A6B4A] text-white border-[#1A6B4A] shadow-sm' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
            }`}>
            <span>{icon}</span> {lbl}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <Card title={TABS.find(t => t[0] === tab)?.[1] || ''} subtitle="Impacto calculado em tempo real">
          <div className="space-y-4">
            {tab === 'contratar' && <>
              <div><label className={LBL}>Salário / honorário (R$)</label>
                <input type="number" className={INP} value={v1} onChange={e => setV1(+e.target.value)}/></div>
            ...
            {tab === 'investir' && <>
              ...
              <div><label className={LBL}>Retorno mensal esperado (R$)</label>
                <input type="number" className={INP} value={v2} onChange={e => setV2(+e.target.value)}/></div>
            </>}
            {tab === 'crescer' && <>
              <div><label className={LBL}>Novos clientes</label>
                <input type="number" className={INP} value={v1} onChange={e => setV1(+e.target.value)}/></div>
              <div><label className={LBL}>Ticket médio</label>
                <input type="number" className={INP} value={v2} onChange={e => setV2(+e.target.value)}/></div>
            </>}
        ...
        <Card title="Resultado por mês" subtitle="Azul = atual · Verde/Vermelho = simulado">
