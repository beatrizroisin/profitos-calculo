...
export default function CeoPage({ searchParams }:{ searchParams:{period?:string} }) {
  ...
  const expenses = stats?.totalExpenses ?? 241856;
  ...
  const qs=[
    {q:'Posso contratar agora?',badge:resultado<0?'no':'maybe',bl:resultado<0?'Não recomendado':'Condicionado',
     ans:resultado<0
    ...
    {q:'Posso fazer um investimento?',badge:'maybe',bl:'Depende do payback',
    ...
    {q:'Vou ficar sem dinheiro?',badge:resultado<0?'no':'maybe',bl:resultado<0?'Risco real':'Monitorar',
     ans:resultado<0
    ...
    {q:'Onde estou errando?',badge:'maybe',bl:'3 pontos críticos',
  ...
  const bs:{[k:string]:string}={yes:'background:#E8F5EF;color:#064E3B',no:'background:#FEE8EA;color:#7F1D1D',maybe:'background:#FEF0E6;color:#78350F'};
    ...
    <div className="space-y-4">
      ...
      {qs.map((p,i)=>(
        <div key={i} className="bg-white border border-gray-100 rounded-2xl p-6">
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium mb-3" style={bs[p.badge] ? {background: bs[p.badge].split(';')[0].split(':')[1], color: bs[p.badge].split(';')[1].split(':')[1]} : {}}>{bi[p.badge]} {p.bl}</div>
