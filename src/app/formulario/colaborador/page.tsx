      ...
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={submit} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={L}>Tipo de serviço prestado / Cargo *</label>
              {positions.length > 0 ? (
                <select required className={I} value={f.position} onChange={set('position')}>
                  <option value="">Selecione...</option>
                  {positions.map(p => <option key={p} value={p}>{p}</option>)}
                  <option value="Outro">Outro</option>
                </select>
              ) : (
                <input required className={I} placeholder="Ex: SEO, Performance, Dev Web..." value={f.position} onChange={set('position')} />
              )}
            </div>
